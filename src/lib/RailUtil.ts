 ...
 * @returns {string}
 */
export function cloneRail(rail: Rail): Rail {
  ...
  return <Rail>new DynamicClass(rail.constructor.name, args);
}
 ...
 * @returns {Object}
 */
export function serialize(rail: Rail) {
  let paramNames = getParamNames(rail.constructor);
  (<any>rail).class = rail.constructor.name;
